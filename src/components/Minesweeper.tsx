...
export default function Minesweeper({
  ...
  const getNumberColor = (num: number) => {
    if (false) {
      switch (num) {
        case 1:
          return "text-sky-800 font-black";
        case 2:
          return "text-emerald-800 font-black";
        case 3:
          return "text-rose-800 font-semibold";
        case 4:
          return "text-violet-850 font-black";
        case 5:
          return "text-amber-900 font-extrabold";
        case 6:
          return "text-teal-900 font-black";
        case 7:
          return "text-[#4A3C31] font-black";
        default:
          return "text-zinc-900 font-black";
      }
    } else {
      switch (num) {
        case 1:
          return "text-[#60A5FA] font-black"; // Vivid blue
        case 2:
          return "text-[#34D399] font-black"; // Lime emerald
        case 3:
          return "text-[#F87171] font-semibold"; // Vibrant red
        case 4:
          return "text-[#C084FC] font-black"; // Purple
        case 5:
          return "text-[#FBBF24] font-extrabold"; // Amber gold
        case 6:
          return "text-[#2DD4BF] font-black"; // Cyan
        case 7:
          return "text-[#F472B6] font-black"; // Rose pink
        default:
      ...
      <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between px-2 py-1">
        ...
        <div className="flex items-center gap-2自适应 justify-end">
          ...
          >
            ...
            <span
              className={`text-xs font-black font-mono leading-none mt-1 ${"text-indigo-400"}`}
            >
          ...
          >
            ...
            >
              ...
              <Clock
                size={8}
                className={isTimerRunning ? "animate-spin" : ""}
        ...
        >
          {board.map((row, r) =>
            row.map((cell, c) => {
              ...
              if (showOpened) {
                if (cell.isMine) {
                  cellBg = "bg-red-950/50 text-red-400";
                } else {
                  // revealed safe block
                  cellBg = false
                    ? isBgOdd
                ...
                cellBg = false
                  ? isBgOdd
                    ? "bg-[#EFEADB] hover:bg-[#E1D6BF]"
                    : "bg-[#E5DFCE] hover:bg-[#D4CBB6]"
                  : isBgOdd
                    ? "bg-zinc-800"
                    : "bg-zinc-850";
              }
              ...
              return (
                <button
                  ...
                  className={`w-full h-full rounded-md flex items-center justify-center relative cursor-pointer font-bold select-none text-xs md:text-sm shadow-inner transition-colors duration-200 border-t border-l ${cellBg} ${
                    false ? "border-[#FAF6EE]/15" : "border-white/5"
                  }`}
                  style={{
                ...
                >
                  ...
                  {showOpened ? (
                    ...
                    <Flag
                      size={cols > 12 ? 10 : 13}
                      className={"text-indigo-400 fill-indigo-500/30"}
                    />
                  ) : null}
                </button>
              );
        ...
        {showGuide && (
          ...
          >
            <p>
              ...
              首次左键点击(挖掘)一个方格时，系统有绝对的防爆逻辑机制，自动为您隔离
              3x3 危险，从而绝不误踩雷。
            </p>
            <p>
              🖱️ <strong className="text-amber-500">提示：</strong>{" "}
              电脑端玩家可以直接右击方格直接标旗。
            </p>
            <p>
              💥{" "}
              ...
              当一个已揭示的数字格周围标记的旗子数量达到对应雷数，您可以
              <span className="underline">双击该数字格</span>
              （或手机上双击），直接瞬间揭示周围所有其它剩余安全格子。
            </p>
          </div>
        )}
      </div>
