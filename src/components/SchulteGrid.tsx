...
export const CAMPAIGN_LEVELS: GameLevel[] = [
  {
    ...
    gridSize: 3,
    timeLimit: null,
    description: "初学试练：3x3 经典方格，静下心，用外周视野全纳九格",
  },
  {
    levelNumber: 2,
    gridSize: 3,
    timeLimit: 10,
    description: "基础提速：3x3 网格，要求在 10 秒时间闸内完成扫视",
  },
  {
    levelNumber: 3,
    gridSize: 4,
    timeLimit: null,
    description: "中阶视宽：4x4 中阶方格，锻炼由中心焦点向四周延展",
  },
  {
    levelNumber: 4,
    gridSize: 4,
    timeLimit: 25,
    description: "极限扫频：4x4 网格，限时 25 秒，逼迫眼球搜索提速",
  },
  {
    levelNumber: 5,
    gridSize: 4,
    timeLimit: 16,
    description: "中阶登顶：4x4 网格，限时 16 秒，进入高扫视反射区",
  },
  {
    levelNumber: 10,
    gridSize: 5,
    timeLimit: 18,
    description: "终极掌控：5x5 大师网格，极限限时 18 秒，跨入天神领域！",
  },
  {
    levelNumber: 6,
    gridSize: 5,
    timeLimit: null,
    description: "5x5 国际标准：测试专注力的经典方格，静候视野全开",
  },
  {
    levelNumber: 7,
    gridSize: 5,
    timeLimit: 50,
    description: "标准测试：5x5 网格，限时 50 秒，测试专注力是否合格",
  },
  {
    levelNumber: 8,
    gridSize: 5,
    timeLimit: 36,
    description: "精锐突刺：5x5 网格，限时 36 秒，训练视网膜边缘阅读力",
  },
  {
    levelNumber: 9,
    gridSize: 5,
    timeLimit: 24,
    description: "超感知觉：5x5 网格，限时 24 秒，大幅缩省大脑解码时差",
  },
];
...
export default function SchulteGrid({
  ...
  // Mode settings
  const [activeTab, setActiveTab] = useState<"level" | "free" | "letter">(
    "level"
  );
  ...
  const [gameState, setGameState] = useState<
    "idle" | "countdown" | "playing" | "completed" | "failed"
  >("idle");
  ...
  const [wrongCellVal, setWrongCellVal] = useState<number | string | null>(
    null
  );
  ...
  const [currentScoreItem, setCurrentScoreItem] = useState<ScoreRecord | null>(
    null
  );
    ...
    <div className="w-full flex flex-col gap-5 select-none text-theme">
      ...
      {onGoBack && (
        <div className="flex items-center justify-between pb-3.5 border-b border-zinc-800/60 w-full px-1">
          <button
            onClick={onGoBack}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-800/60 border border-zinc-700/50 hover:bg-zinc-800 hover:text-white text-secondary transition-all text-xs font-semibold cursor-pointer active:scale-95"
          >
            <ArrowLeft size={13} />
            <span>返回大厅</span>
          </button>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[11px] text-muted font-mono tracking-wider block uppercase">
              舒尔特专注格盘 (Retina)
            </span>
          </div>
        </div>
      ...
      <div
        className={`p-4 md:p-6 rounded-2xl card-theme transition-all duration-300 w-full`}
      >
        ...
        {gameState === "idle" && (
          <div>
              ...
              <div className="space-y-4">
                <div className="bg-zinc-900/60 rounded-xl p-4 border border-zinc-800 flex items-center justify-between gap-3 flex-col sm:flex-row">
                  ...
                  <div className="flex items-center gap-1 mr-1 shrink-0 text-amber-500 font-mono text-xs">
                    <Hourglass size={13} />
                    <span>
                      {currentLevelInfo.timeLimit
                        ? `限时 ${currentLevelInfo.timeLimit} 秒`
                        : "无时间限制"}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <span
                    className={`text-[10px] text-muted uppercase block mb-2 font-bold tracking-wider`}
                  >
                    关卡选择 (已解锁):
                  </span>
                      ...
                      <button
                        ...
                        className={`py-2 text-xs font-mono font-bold rounded-lg transition-all border ${
                          index === currentLevelIdx
                            ...
                            : "bg-zinc-900 border-zinc-800 text-secondary hover:bg-zinc-800 hover:border-zinc-700"
                        }`}
                      >
                        {lvl.levelNumber}
                      </button>
            ...
            {activeTab === "free" && (
              <div className="space-y-4">
                <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-xl p-4">
                  <span
                    className={`text-xs text-muted block mb-2 font-semibold`}
                  >
                      ...
                      <button
                        ...
                        className={`py-2.5 rounded-lg font-mono font-bold text-xs transition-all border ${
                          freeSize === size
                            ...
                            : "bg-zinc-900 border-zinc-800 text-secondary hover:border-zinc-700"
                        }`}
                      >
                        {size} x {size}
                      </button>
                  ...
                  <div className="mt-4 p-3 bg-amber-500/5 border border-amber-500/10 rounded-lg text-xs leading-relaxed text-secondary">
                    ⚡ 5x5 是公认的黄金诊断配置，6x6
                    以上极度考验眼角周边外膜视觉。建议挺拔坐姿，双眼距离屏幕 30
                    厘米以上，固定直视中央！
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-zinc-900/60 rounded-xl p-4 border border-zinc-800">
                  ...
                  <p className={`text-xs text-muted mt-1.5 leading-relaxed`}>
                    ...
                    并将余下的 25 个字母随机洗牌。要求你按照 A-B-C
                    的纯字母顺序连续指认。这不仅是扫眼测试，更是在由于未知缺失而导致决策犹豫时，诊断心理抗干扰力的绝佳方案！
                  </p>

                  <div className="mt-3.5 flex items-center gap-2 p-2 px-3 bg-zinc-950 rounded border border-zinc-800 text-[11px] text-secondary">
                    <CheckCircle size={14} className="text-emerald-500" />
                    <span>
                      顶端状态栏会自动提醒下一个搜寻字母，免去记诵顾虑。
                    </span>
                  </div>
                </div>
              </div>
            ...
            <button
              ...
              className={`w-full py-4 mt-6 rounded-xl text-sm font-bold flex items-center justify-center gap-2 btn-accent`}
            >
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="w-full flex items-center justify-between border-b border-zinc-800/80 pb-3 mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {activeTab === "level" &&
                  CAMPAIGN_LEVELS[currentLevelIdx].timeLimit && (
            ...
            {activeTab === "letter" && (
              <div className="w-full mb-3 text-center text-[10px] text-muted bg-zinc-950/40 p-1.5 rounded transition-all">
            ...
            <div className="w-full text-left text-[10px] text-muted flex items-center justify-between mb-4 px-1">
              <span>
                第 {targetIndex} / {sortedTargets.length} 项
              </span>
              <span>
                失误:{" "}
                <b className={mistakes > 0 ? "text-red-400" : ""}>{mistakes}</b>{" "}
                次
              </span>
            </div>
            ...
            >
              {gridCells.map((val, cellIdx) => {
                ...
                const isCellActivated =
                  targetIdxOfVal !== -1 && targetIdxOfVal < targetIndex;
                ...
                const isInActiveTrail =
                  isCellActivated && targetIndex - 1 - targetIdxOfVal < 5;
                const clickAge = isCellActivated
                  ? targetIndex - 1 - targetIdxOfVal
                  : 999;
                ...
                if (isCellActivated) {
                  if (isInActiveTrail) {
                    ...
                    const scales = [
                      "scale-105 shadow-md",
                      "scale-102 shadow-sm font-black",
                      "scale-100",
                      "scale-98 opacity-90",
                      "scale-95 opacity-80",
                    ];

                    const opacity = opacities[clickAge] || 0.3;
                    const scaleClass = scales[clickAge] || "scale-95";

                    customStyle = `grid-cell-active ${scaleClass}`;
                    inlineStyle = {
                      ...inlineStyle,
                      opacity: opacity,
                    };
                    ...
                    customStyle = `grid-cell select-none cursor-default opacity-15 pointer-events-none border-transparent shadow-none scale-95`;
                  }
                } else if (isWrong) {
                  customStyle = "grid-cell-wrong";
                } else {
                  customStyle = `grid-cell active:scale-95 cursor-pointer shadow-sm hover:scale-102`;
                }
            ...
            <button
              ...
              className="mt-6 flex items-center gap-1 text-xs text-muted hover:text-zinc-300 py-2 px-4 rounded hover:bg-white/5 transition-all active:scale-95 border border-zinc-800"
            >
              <RotateCcw size={12} />
              <span>退出本次练习 (清零)</span>
            </button>
          ...
          <div className="text-center py-8">
            ...
            <div className="my-6 py-4 px-6 rounded-xl bg-zinc-950/60 border border-zinc-800 w-full max-w-sm mx-auto">
              <span className={`text-xs text-muted block`}>
                本次完成耗时 (
                {activeTab === "level"
                  ? `第${CAMPAIGN_LEVELS[currentLevelIdx].levelNumber}关`
                  : activeTab === "free"
                    ? `${freeSize}x${freeSize}`
                    : "字母款"}
                )
              ...
              <div className="mt-3 pt-3 border-t border-zinc-900 grid grid-cols-2 text-xs text-secondary">
                <div>
                  <span>失误次数</span>
            ...
            <div className="flex flex-col gap-2.5 max-w-sm mx-auto">
              ...
              <button
                ...
                className="py-3 px-4 rounded-xl text-xs font-bold text-secondary hover:text-white bg-zinc-900 border border-zinc-800 transition-all active:scale-95 cursor-pointer"
              >
                返回挑战大厅 (选关/模式)
              </button>
            </div>
        ...
        {gameState === "failed" && (
          <div className="text-center py-10">
            ...
            >
              本关具有严格眼球扫频限重（{currentLevelInfo.timeLimit}
              秒）。别灰心，深呼吸，平静心境能大幅提高摄入网格速度！
            </p>
            ...
            <div className="mt-8 flex flex-col gap-2 max-w-xs mx-auto">
              <button
                id="retry-failed-btn"
                onClick={startChallenge}
                className={`py-3 px-4 rounded-xl text-xs font-bold btn-accent`}
              >
                重新发起挑战
              </button>
              ...
              <button
                ...
                onClick={() => setGameState("idle")}
                className="py-3 px-4 rounded-xl text-xs font-medium text-secondary hover:text-white bg-zinc-900 border border-zinc-800 transition-all"
              >
                返回训练大厅
              </button>
            </div>
