...
export default function Leaderboard({
  ...
  const [leaderboardMode, setLeaderboardMode] = useState<"entropy" | "games">(
    "entropy"
  );
      ...
      try {
        if (leaderboardMode === "entropy") {
          ...
          if (selectedGame === "schulte" && selectedDifficulty) {
            ...
            const records = await fetchGameScores(
              selectedGame,
              selectedDifficulty,
              15
            );
  ...
  const getMergedScores = (): ScoreRecord[] => {
    ...
    if (selectedGame === "schulte" && selectedDifficulty) {
      ...
      challengers.forEach((chal) => {
        if (
          !allRecords.some((r) => r.id === chal.id || r.userId === chal.userId)
        ) {
  ...
  const getMergedEntropyLeaderboard = (): any[] => {
    ...
    if (user && user.userId) {
      ...
      if (selfIndex === -1 && (user.negentropy || 0) > 0) {
        ...
        selfIndex !== -1 &&
        (user.negentropy || 0) > (allEntropy[selfIndex].negentropy || 0)
      ) {
    ...
    return allEntropy
      .sort((a, b) => (b.negentropy || 0) - (a.negentropy || 0))
      .slice(0, 10);
  };
  ...
  return (
    <div className={`p-5 rounded-2xl card-theme transition-all duration-300`}>
      {/* Header */}
        ...
        <div>
          <div className="flex items-center gap-2">
            <Trophy
              ...
              size={18}
            />
            <h3 className={`text-base font-black text-theme`}>
              格子熵 · 巅峰荣誉殿堂
            </h3>
          </div>
      ...
      <div className="grid grid-cols-2 p-1 rounded-xl bg-black/60 border border-zinc-900 text-xs gap-1 mb-5">
        ...
        >
          <Orbit
            size={13}
            className={leaderboardMode === "entropy" ? "animate-spin" : ""}
      ...
      {leaderboardMode === "entropy" && (
        <div className="space-y-2">
          {finalEntropyLeaderboard.map((record, index) => {
            ...
            if (isSelf)
              cardBorder =
                "border-[#3EB489]/50 bg-[#3EB489]/5 text-[#3EB489] shadow-sm";
            ...
            return (
              ...
              >
                ...
                <div className="text-right flex flex-col justify-center">
                  <span
                    className={`font-mono text-xs font-black leading-none ${isSelf ? "text-[#3EB489]" : "text-[#3EB489]/90"}`}
                  >
