...
export default function Gomoku({ 
  ...
  const [board, setBoard] = useState<Board>(() =>
    Array(initialGridSize).fill(null).map(() => Array(initialGridSize).fill(null))
  );
  const [currentPlayer, setCurrentPlayer] = useState<'black' | 'white'>('black'); // Black plays first
  const [gameMode, setGameMode] = useState<'vs-bot' | 'pass-and-play'>('vs-bot');
  ...
  useEffect(() => {
    ...
    setBoard(Array(initialGridSize).fill(null).map(() => Array(initialGridSize).fill(null)));
  ...
  const resetGame = (sizeToUse: number = gridSize) => {
    setBoard(Array(sizeToUse).fill(null).map(() => Array(sizeToUse).fill(null)));
    setCurrentPlayer('black');
    setWinner(null);
    setWinLine([]);
    setHasConsumedEntropy(false);
  ...
  useEffect(() => {
    ...
    if (onConsumeEntropy) {
      ...
      if (winner === 'black' && gameMode === 'vs-bot') {
        ...
        onConsumeEntropy(difficulty === 'easy' ? Math.floor(reward * 0.7) : difficulty === 'hard' ? Math.floor(reward * 1.3) : reward);
      } else {
        onConsumeEntropy(15); // Mental effort contribution
      }
    }
  }, [winner, gameMode, gridSize, difficulty, onConsumeEntropy, hasConsumedEntropy]);

  // Check victory condition checking for 5 consecutives in 4 directions
  const checkWin = (b: Board, row: number, col: number, player: 'black' | 'white'): [number, number][] | null => {
    const directions = [
      [0, 1],   // horizontal
      [1, 0],   // vertical
    ...
    for (const [dr, dc] of directions) {
      ...
      let r = row + dr;
      let c = col + dc;
      while (r >= 0 && r < gridSize && c >= 0 && c < gridSize && b[r][c] === player) {
        line.push([r, c]);
        r += dr;
        c += dc;
      }
    ...
    const evaluatePoint = (row: number, col: number): number => {
      ...
      for (const [dr, dc] of dirs) {
        ...
        const countStones = (player: 'black' | 'white') => {
          ...
          if (r >= 0 && r < gridSize && c >= 0 && c < gridSize && b[r][c] === null) {
            openEnds++;
          }
        ...
        else if (playerStats.consecutive === 4 && playerStats.openEnds > 0) score += 10000;
        // 3. Complete bot 4-in-a-row
        else if (botStats.consecutive === 4 && botStats.openEnds > 0) score += 8000;
        // 4. Block player 3-in-a-row (open ends)
        else if (playerStats.consecutive === 3 && playerStats.openEnds === 2) score += 5000;
        // 5. Build bot 3-in-a-row
        else if (botStats.consecutive === 3 && botStats.openEnds === 2) score += 3000;
        // 6. Generic small weights
        else {
          score += botStats.consecutive * (difficulty === 'hard' ? 12 : 10) + botStats.openEnds * 5;
          score += playerStats.consecutive * (difficulty === 'hard' ? 10 : 8) + playerStats.openEnds * 4;
        }
      }
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    if (winningTrail) {
      ...
      confetti({
        ...
        spread: 80,
        origin: { y: 0.6 },
        colors: player === 'black' ? ['#10B981', '#3EB489', '#FFD700', '#ffffff'] : ['#E4E4E7', '#FBBF24', '#3B82F6']
      });
    } else if (checkDraw(nextBoard)) {
      setWinner('draw');
    } else {
  ...
  const stoneSizeClass = 
    gridSize === 11 ? 'w-6.5 h-6.5 sm:w-8 sm:h-8' :
    gridSize === 13 ? 'w-5.5 h-5.5 sm:w-7 sm:h-7' :
    'w-4.5 h-4.5 sm:w-6 sm:h-6';

  const hoverSizeClass = 
    gridSize === 11 ? 'w-4 h-4 sm:w-6 sm:h-6' :
    gridSize === 13 ? 'w-3.5 h-3.5 sm:w-5 sm:h-5' :
    'w-3 h-3 sm:w-4 sm:h-4';

  return (
    <div className="w-full flex flex-col gap-4 select-none text-zinc-300">
      ...
      <div className="flex items-center justify-between pb-3.5 border-b border-zinc-800/60 w-full px-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-zinc-400 font-mono tracking-wider block uppercase font-bold">
            五子棋连珠博弈 ({gridSize}x{gridSize}) - {difficulty === 'easy' ? '入门' : difficulty === 'hard' ? '大师' : '普通'} AI
          </span>
        </div>
      </div>

      {/* Dynamic Game-settings Panel directly inside Go Interface */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-zinc-900/35 border border-zinc-805/40 p-3 rounded-xl">
        {/* Row 1: Board specifications choice */}
        <div className="flex items-center justify-between gap-2.5 bg-zinc-950/25 p-2 rounded-lg border border-zinc-900/65">
          <span className="text-xs font-bold text-zinc-400 font-mono tracking-wide shrink-0">棋盘规格:</span>
          <div className="flex items-center gap-1.5">
            {[11, 13, 15].map((size) => (
              <button
                key={`opt-size-${size}`}
        ...
        <div className="flex items-center justify-between gap-2.5 bg-zinc-950/25 p-2 rounded-lg border border-zinc-900/65">
          <span className="text-xs font-bold text-zinc-400 font-mono tracking-wide shrink-0">对手智商:</span>
          <div className="flex items-center gap-1.5">
            {[
              { id: 'easy', label: '入门' },
              { id: 'medium', label: '普通' },
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/60">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3.5 px-1.5 py-0.5">
          <div className="flex items-center gap-2">
            <span className="text-xs text-zinc-500">落子权:</span>
            
            <div className="flex items-center gap-1.5">
              <div className={`w-3.5 h-3.5 rounded-full border border-zinc-750 shadow-sm ${
                currentPlayer === 'black' ? 'bg-zinc-950 text-white border-zinc-850' : 'bg-white text-zinc-950 border-zinc-300'
              }`} />
              <span className="text-xs font-bold text-zinc-300 font-mono">
                {currentPlayer === 'black' ? `${userNickname} (黑子)` : gameMode === 'vs-bot' ? '智能AI (白子)' : '同屏对手 (白子)'}
              </span>
            </div>
          </div>
          ...
          <button
            onClick={() => resetGame()}
            className="p-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-750 border border-zinc-700/40 text-zinc-400 hover:text-white transition-all cursor-pointer"
            title="重开棋局"
          >
            <RefreshCw size={12} />
          </button>
      ...
      >
        ...
        <div className="absolute inset-3 pointer-events-none opacity-40">
          ...
          {gridSize % 2 === 1 && (
            <>
              {(() => {
                const center = Math.floor(gridSize / 2);
                const starPoints: [number, number][] = gridSize === 11 
                  ? [[2, 2], [2, 8], [8, 2], [8, 8], [center, center]]
                  : gridSize === 13 
                  ? [[3, 3], [3, 9], [9, 3], [9, 9], [center, center]]
                  : [[3, 3], [3, 11], [11, 3], [11, 11], [center, center]];
          ...
          {board.map((row, rIdx) =>
            row.map((val, cIdx) => (
              <button
                ...
                disabled={val !== null || winner !== null || (currentPlayer === 'white' && gameMode === 'vs-bot')}
              ...
              >
                ...
                {val === null && !winner && !(currentPlayer === 'white' && gameMode === 'vs-bot') && (
                  <div className={`absolute select-none pointer-events-none rounded-full opacity-0 group-hover:opacity-35 border border-dashed border-zinc-950 ${hoverSizeClass} ${
                ...
                {val !== null && (
                  <motion.div
                    ...
                    className={`absolute rounded-full flex items-center justify-center shadow-md select-none ring-1 relative ${stoneSizeClass} ${
                      val === 'black'
                        ? 'bg-gradient-to-br from-zinc-800 to-zinc-950 text-white ring-zinc-900/60 border border-zinc-900'
                        : 'bg-gradient-to-br from-white to-zinc-150 text-zinc-900 ring-zinc-300 border border-zinc-100/90'
                    }`}
                  >
                    ...
                    {val === 'black' && isWinPoint(rIdx, cIdx) && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0 z-20" />}
                    {val === 'white' && isWinPoint(rIdx, cIdx) && <div className="w-1.5 h-1.5 rounded-full bg-emerald-600 shrink-0 z-20" />}
                  </motion.div>
                )}
              </button>
            ))
        ...
        <AnimatePresence>
          {winner && (
            ...
            >
              <Swords size={38} className="text-amber-500 mb-3.5 animate-bounce" />
              <h3 className="text-lg font-black text-white uppercase tracking-wider">棋局终了</h3>
              <p className="text-xs text-zinc-300 mt-2 max-w-[280px]">
                {winner === 'draw' ? (
                  <span>落子无处！此局以棋逢对手「和棋」落帷。</span>
                ) : winner === 'black' ? (
                  <span>恭喜 <span className="text-[#3EB489] font-black">{userNickname} (黑子)</span> 连成五子，赢得本次博弈大捷！</span>
                ) : (
                  <span>
                    {gameMode === 'vs-bot' ? (
                      <span>白子 AI 技高一筹完成五连珠！<br/><span className="text-zinc-500 mt-1 block">其落子诡橘，下次小心陷入它的陷阱。</span></span>
                    ) : (
      ...
      {/* Guide text */}
      <div className="bg-zinc-900/30 p-3 rounded-lg border border-zinc-800/40 text-[11px] text-zinc-500 leading-relaxed text-center font-mono select-text">
        💡 五子棋规则：黑先白后，双方轮流落子，先在横、竖、斜任意方向连成连续 5 颗同色棋子者胜。消解越大棋盘规格或更高对手智商将获得更多负熵。
      </div>
