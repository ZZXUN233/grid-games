...
export default function NameEditor({ user, onChange, theme }: NameEditorProps) {
  ...
  const handlePasswordChange = async () => {
    ...
    if (newPassword.length < 6) { setPwdError('新密码至少 6 个字符'); return; }
  ...
  return (
    <div className={`p-4 rounded-xl ${theme.card} transition-all duration-300`}>
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="relative group">
          <div
            ...
            title="点击更换形象"
          >
            <span className="relative z-10 select-none animate-float">{user.avatarEmoji}</span>
            <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <span className="text-[10px] text-white font-medium bg-black/60 px-1 rounded">更 换</span>
            </div>
          </div>
          {avatarLoading && (
            <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 text-[8px] text-emerald-400 animate-pulse">同步中</span>
          )}
        </div>
        ...
        <div className="flex-1 w-full text-center sm:text-left">
          <div className="flex flex-col justify-center sm:justify-start">
            <span className={`text-xs ${theme.textMuted} font-mono tracking-wider mb-0.5`}>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2 mt-1">
              {isEditing ? (
                ...
                <div className="flex items-center gap-2 group">
                  <h3 className={`text-lg font-bold ${theme.textMain}`}>{user.nickname}</h3>
                  <button
                    ...
                    className={`p-1 rounded-md opacity-30 group-hover:opacity-100 hover:bg-white/10 transition-all ${theme.textMuted}`}
                    title="修改昵称"
                  >
                    <Edit2 size={13} />
                  </button>
      ...
      <AnimatePresence>
        {showColorPicker && (
          <motion.div
            ...
            className={`mt-4 pt-4 border-t ${theme.border} overflow-hidden`}
          >
            <div>
              <span className={`text-xs ${theme.textMuted} block mb-1.5 font-medium`}>选择头像底色:</span>
                ...
                {AVATAR_COLORS.map((color) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: user.avatarColor === color ? `0 0 0 2px ${color}` : 'none'
                    }}
                  />
                ))}
              </div>
            ...
            <div className="mt-3">
              <span className={`text-xs ${theme.textMuted} block mb-1.5 font-medium`}>选择胸章图标:</span>
              <div className="flex flex-wrap gap-2">
                {AVATAR_EMOJIS.map((emoji) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded flex items-center justify-center text-lg cursor-pointer bg-white/5 hover:bg-white/10 active:scale-95 transition-all ${
                      user.avatarEmoji === emoji ? 'ring-1 ring-amber-500 bg-white/10' : ''
                    }`}
                  >
                    {emoji}
                  </button>
      ...
      <div className="mt-4 pt-3 border-t border-zinc-800/30">
        <button
          onClick={() => { setShowPasswordChanger(!showPasswordChanger); setPwdError(''); setPwdSuccess(''); }}
        ...
        <AnimatePresence>
          {showPasswordChanger && (
            ...
            >
              ...
              {pwdSuccess && (
                <div className={`p-2 rounded-lg border text-[10px] ${
                  isLight ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-emerald-950/40 border-emerald-500/40 text-emerald-400'
                }`}>
                  {pwdSuccess}
                </div>
              )}
