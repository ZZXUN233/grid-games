...
import {
  Play, Pause, StepForward, RotateCcw, Sparkles, ArrowLeft, Flame,
  Dna, SkipForward, Info
} from 'lucide-react';
...
const PRESETS: Record<string, { name: string; cells: [number, number][] }> = {
  ...
  beacon: {
    name: '信标 (Beacon)',
    cells: [[1,1],[1,2],[2,1],[2,2],[3,3],[3,4],[4,3],[4,4]],
  },
  toad: {
    name: '蛤蟆 (Toad)',
...
export default function GameOfLife({ theme, onGoBack, userNickname, onConsumeEntropy }: GameOfLifeProps) {
  ...
  useEffect(() => {
    if (stableGenerationsRef.current >= 20 && !entropyAwarded &&
  ...
  const txtMuted = isLight ? 'text-[#6C5E53]' : 'text-zinc-400';
  const txtMain = isLight ? 'text-[#4A3C31]' : 'text-white';
  const borderCls = isLight ? 'border-[#DFD3C1]' : 'border-zinc-800';
  const bgCard = isLight ? 'bg-[#FAF6EE]' : 'bg-zinc-900/40';

  return (
    <div className={`w-full flex flex-col gap-4 select-none ${isLight ? 'text-[#4A3C31]' : 'text-zinc-300'}`}>
      {/* Header */}
      <div className={`flex items-center justify-between pb-3.5 border-b w-full px-1 ${borderCls}`}>
        ...
        <div className="flex items-center gap-1.5">
          <div className={`w-2.5 h-2.5 rounded-full ${running ? 'bg-emerald-500 animate-pulse' : stableCheck ? 'bg-amber-500' : 'bg-zinc-500'}`} />
          <span className={`text-[11px] font-mono tracking-wider uppercase ${txtMuted}`}>
            元胞生活 · LIFE
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 px-2 py-1">
        <div className="flex items-center gap-1">
          ...
          <span className={`text-xs font-bold font-mono ${txtMain}`}>{userNickname}</span>
        </div>
        <div className={`px-2 py-1 rounded-lg border ${borderCls} ${bgCard}`}>
          <span className={`text-[9px] font-mono font-bold ${isLight ? 'text-[#8B5A2B]' : 'text-emerald-400'}`}>
            ♾ {generation} 代
          </span>
        </div>
        <div className={`px-2 py-1 rounded-lg border ${borderCls} ${bgCard}`}>
          <span className={`text-[9px] font-mono font-bold ${isLight ? 'text-[#8B5A2B]' : 'text-indigo-400'}`}>
      ...
      {/* Controls */}
      <div className={`flex items-center justify-center gap-2 p-2 rounded-xl border ${borderCls} ${bgCard} max-w-sm mx-auto w-full`}>
        <button onClick={() => setRunning(!running)}
          className={`p-2 rounded-lg text-xs font-black cursor-pointer transition-all active:scale-90 ${
            running
              ...
              : isLight ? 'bg-[#8B5A2B] text-white' : 'bg-emerald-500 text-zinc-950'
          }`}
        >
        ...
        <button onClick={() => { setRunning(false); loadPreset('random'); }}
      ...
      <div className="flex justify-center">
        <div className="inline-block p-1.5 rounded-2xl border"
          style={{ backgroundColor: isLight ? '#DFD3C1' : '#09090B', borderColor: isLight ? '#DFD3C1' : 'rgba(39,39,42,0.5)' }}
        >
          <div className="grid gap-[1px]" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 1fr)` }}>
            {grid.map((row, r) =>
              row.map((alive, c) => (
                <div key={`${r}-${c}`}
                  ...
                  style={{
                    ...
                    backgroundColor: alive
                      ? (isLight ? '#4A3C31' : '#34D399')
                      : (isLight ? '#EFEADB' : '#18181B'),
                    boxShadow: alive
                      ? `0 0 4px ${isLight ? 'rgba(74,60,49,0.3)' : 'rgba(52,211,153,0.25)'}`
                      : 'none',
                  }}
                />
              ))
      ...
      {entropyAwarded && (
        <div className={`text-center text-[10px] font-bold py-1 ${isLight ? 'text-emerald-600' : 'text-emerald-400'} animate-pulse`}>
          已达成稳定环状态，负熵已消解
        </div>
      )}

      {/* Guide */ }
      <div className={`max-w-sm mx-auto w-full p-2.5 rounded-xl border ${borderCls} ${bgCard}`}>
        <div className="flex justify-between items-center">
          <span className={`text-[9px] uppercase tracking-widest font-black ${txtMuted}`}>玩法说明</span>
        ...
        {showGuide && (
          <div className={`mt-1.5 p-2 rounded-lg border text-[10px] leading-relaxed ${isLight ? 'bg-[#FAF6EE] border-[#E1D4C0] text-[#6C5E53]' : 'bg-zinc-950/40 border-zinc-900 text-zinc-400'}`}>
            <p>🧬 <strong>生命游戏：</strong> 每个元胞的生死取决于相邻元胞数。活元胞 2-3 个邻居存活，死元胞恰好 3 个邻居则新生。</p>
            <p>🎭 <strong>预设：</strong> 滑翔机、脉冲器、高斯帕机枪等经典模式可直接加载观赏。</p>
            <p>� <strong>熵奖励：</strong> 在冥想观测模式下，演化达到 50/100/200 代并进入稳定环状态时，结算渐进式负熵抗熵回馈。</p>
          </div>
        )}
      </div>
    </div>
